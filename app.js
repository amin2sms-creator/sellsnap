...
function loadMoreColors() {
  ...
  const start = currentColorPage * COLORS_PER_PAGE;
...
function loadMoreTemplates() {
  ...
  if (end >= ALL_TEMPLATE_IDS.length) {
    loadMoreBtn.style.display = "none";
  }
}
const TRANSLATIONS = {
...
function currentGuideStep() {
  ...
  if (!hasDetails) {
    return {
      step: "product",
      title: "Replace demo details",
      detail: "Enter your real product name, price, and contact.",
      action: "Edit Details"
    };
  }
  ...
  if (!hasPhoto) {
    return {
      ...
      detail: "Use a clear image for a stronger poster.",
      action: "Choose Photo"
    };
  }
  ...
  if (!hasDesign) {
    return {
      ...
      detail: "Pick a layout that matches the offer.",
      action: "Open Design"
    };
  }
  ...
  if (!hasCampaign) {
    return {
      ...
      detail: "Prepare copy and export formats together.",
  ...
  return {
    step: "export",
    title: "Export poster",
    detail: "This downloads one poster. Kit > Pack creates platform-specific files.",
